...
bool Game::combat(Character monstre)
{
	...
	while (finCombat!=true)
	{
		...
		sortJoueur=selectSkillJoueur(finCombat);
...
Skill Game::selectSkillJoueur(bool &finCombat)
{
...
Skill Game::selectSkillMonstre(Character monstre)
{
	bool done=false; //vérifie si la compétence a bien été choisie
	Skill sort;
	if (monstre==NULL)	//Si null en argument choix Skill joueur
	{
		...
		while (done!=true)
		{
			...
			else cerr<<"Pas assé de mana"<<endl;
		}
	}
	else		//Si monstre != null alors on choisie la compétence du mob
	{
		while (!done)
		{
			srand (time(NULL));
